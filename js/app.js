...
cardapio.metodos = {
    ...
    // 2. Aplica a máscara baseada na quantidade de números digitados
    if (valor.length > 9) {
        ...
        // Formato: 000 (enquanto digita os primeiros 3)
        input.value = valor;
    }
},
    ...
    if (valor.length <= 10) {
        // Formato para fixo: (11) 4444-4444
    ...
    // Se quiser disparar algo quando completar o celular (11 dígitos + máscara = 15 caracteres)
    if (input.value.length === 15) {
        // cardapio.metodos.acaoAposTelefone();
    }
},
	...
	carregarBotaoWhatsap: () => {
		$('.botao-whatsapp').attr(
			'href',
			`https://wa.me/${CELULAR_EMPRESA}?text=Olá, preciso de ajuda com meu imposto de renda.`,
		);
	},
